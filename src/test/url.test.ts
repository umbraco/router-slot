...
describe('url', () => {
  ...
  it('[basepath] should return correct base path', () => {
    const basePaths = [
      ...
      [
        `https://cdpn.io/boomboom/v2/index.html?key=iFrameKey-ca757c8e-dad1-d965-1aed-7cabdaa22462`,
        `/boomboom/v2/`,
      ],
    ];
  ...
  it('[toQuery] should return the correct query object', () => {
    expect(JSON.stringify(toQuery('test=1234&redirect'))).to.equal(
      JSON.stringify({ test: '1234', redirect: '' })
    );
  });

  it('[toQueryString] should return the correct query string', () => {
    expect(toQueryString({ test: '1234', redirect: '' })).to.equal(
      'test=1234&redirect'
    );
  });
});
